#Capstone project to determine who murdered Jay
...
class TextSample:
    ...
    def __repr__(self):
        return "{}, text sample average sentence length: {} words".format(self.author, self.average_sentence_length)

def get_average_sentence_length(text):
    sentences_in_text = []
    text = text.replace("?", ".").replace("!", ".").replace(",", " ").rstrip(".").split(".")
    sentences_in_text = text
    sentences_length = []
    for sentence in sentences_in_text:
        sentences_length.append(len(sentence.split()))
        average = (sum(sentences_length) / len(sentences_length))
    return average
...
def prepare_text(text):
    prepared_text = []
    prepared_text = text.lower().replace("?", " ").replace("!", " ").replace(".", " ").replace("'", " ").replace(",", " ").split() #remove all punctuation and force to lowercase
...
def frequency_comparison(table1, table2):
    ...
    for key in table1:
        if key in table2:
            appearances += max(table1[key], table2[key]) #adding the larger value to the appearances dict
            mutual_appearances += min(table1[key], table2[key])#adding the smaller value to the mutual_appearances dict
        else:
            appearances += table1[key] #if no key match, add the value for the key into appearances dict
    
    for key in table2:  #iterating through the rest of the keys in table2 and adding them to appearances dict
...
for suspect in suspects:
    print('name: {}, intro similarity compared to murder letter: {}'
          .format(suspect.author, find_text_similarity(murderer_sample, suspect)))  #looping through the 3 suspects to return their scores
...
print("The murderer is...duh, duh, duh...Myrtle Beech") #her score was the closest to the murderer_sample